...
collection_start = """
Капитан **{captain_name}** выплывает на морские ежедневки с Око Окиллы в 
**{time_leaving}** на канале **{server}**. 
Желающие присоединиться к кэпу должны нажать на :heart:. 
И обязательно, посмотрите сообщение, 
которое я вам выслал в личные сообщения, может быть вы не попали в рейд. 
Мест осталось {places_left}. 
Обновлённая таблица появится скоро.
"""
...
raid_leave = """
f"Я тебя удалил из списка на ежедневки с капитаном **{captain_name}**"
"""
