...
from bdo_daily_bot.core.users_interactor.message_reaction_interactor import MessageReactionInteractor
...
class RaidNotifier:
    ...
    async def notify_about_leaving(cls, raid: Raid):
        ...
        if secs_before_notification < 0:
            logging.info(
                "Raid {}/{}: Members will not be notified. Little time left before raid left".format(
        ...
        logging.info(
            "Raid {}/{}: Members was notified about leaving".format(
                raid.captain.nickname, raid.time.normal_time_leaving
            )
        )
    ...
    async def __notify_captain_about_leaving(cls, captain_name: str):
        ...
        if not captain_document:
            logging.warning(
                "Raid {}/_: Can't send notification message to captain. "
    ...
    @classmethod
    def __users_generator(
        cls, raid_members_documents: List[Dict[str, str]]
        ...
        for user_document in raid_members_documents:
            ...
            if user_id := user_document.get("discord_id"):
                ...
                logging.warning("User document don't contain user id. Document content:\n{}".format(user_document))
