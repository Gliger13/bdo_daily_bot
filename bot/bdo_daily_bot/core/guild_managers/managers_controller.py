...
from bdo_daily_bot.core.guild_managers.raids_manager import RaidsGuildManager
from bdo_daily_bot.core.raid.raid import Raid
from bdo_daily_bot.core.raid.raid_channel import RaidChannel
from bdo_daily_bot.core.raid.raid_flow import RaidFlow
from bdo_daily_bot.core.raid.raid_item import RaidItem
from bdo_daily_bot.core.raid.raid_item_factory import RaidItemFactory
...
class ManagersController:
    ...
    async def load_raids(cls):
        ...
        for raid in raids:
            if raid.channels:
                ...
                logging.warning(
                    "Bot initialisation: Raid {}/{}: Channels from the database are empty. "
                    "Can't load raid raid. Need to manually remove the defect document.".format(
                        raid.captain.nickname, raid.time.normal_time_leaving
                    )
                )
                continue
            asyncio.ensure_future(cls.__start_raid_flow(raid))
            logging.info(
                "Bot initialisation: Raid {}/{}: Raid from the database was loaded and started.".format(
                    raid.captain.nickname, raid.time.kebab_time_leaving
                )
            )
    ...
    async def __start_raid_flow(cls, raid_to_start: Raid):
        ...
        logging.info(
            "Raid {}/{}: Raid was completely ended.".format(
                raid_to_start.captain.nickname, raid_to_start.time.kebab_time_leaving
            )
        )
