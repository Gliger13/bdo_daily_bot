...
help_msg_dm_invite_reg = ("Массовый спам\n"
                          ...
                          " зарегистрироваться через команду ```css!!рег```."
                          "\nНужно обладать ролью Капитан для использование этой команды")
help_msg_load_raid = ("Загружает состав последнего рейда\n"
                      ...
                      "Например: ```css!!загрузи_рейд Хлебушек 22:30``` - загружает состав рейда капитана Хлебушек с\n"
...
help_msg_reg = ("Регистрация игровой фамилии в базе данных.\n Позволяет использовать смайлик для бронирования\n"
                ...
                "Например: ```css!!рег Хлебушек```. В таблице, после нажатия смайлика, вы будете подписаны *Хлебушек*")
help_msg_rereg = ("Регистрация новой игровой фамилии в базе данных.\n"
                  "Например: ```css!!перерег Хлебушек```. В таблице, после нажатия смайлика,"
                  " вы будете подписаны *Хлебушек*")
help_msg_reserve = ("Устаревшая команда. Позволяет засунуть человека в рейд.\n"
                    "Например: ```css!!бронь Хлебушек Хомяк```. Засунет *Хлебушек* в рейд капитана *Хомяк*\n"
                    "Ещё одно применение: ```css!!бронь Хлебушек```."
...
help_msg_remove_res = ("Устаревшая команда. Удаляет человека из рейда.\n "
                       ...
                       "Например: ```css!!удали_бронь Хлебушек```\n"
                       "Нужно обладать ролью Капитан для использование этой команды")
help_msg_show_raids = "Показывает все рейды, которые ещё не уплыли"
help_msg_show = ("Показывает таблицу с участниками рейда\n" 
                 "Например: ```css!!покажи Хомяк```. Показывает таблицу состава рейда капитана Хомяк\n" 
                 "```css!!покажи Хомяк 22:30```. Показывает таблицу капитана Хомяк, который отплывает в 22:30\n"
                 "Нужно обладать ролью Капитан для использование этой команды")
help_msg_remove_raid = ("Удаляет рейд.\n"
                        "Например: ```css!!удали_рейд Хомяк``` - удаляет рейд капитана Хомяк\n"
                        "```css!!удали_рейд Хомяк 22:30``` - удаляет рейд капитана Хомяк, который отплывает в 22:30\n"
                        "Нужно обладать ролью Капитан для использование этой команды")
help_msg_collection = ("Открывает бронирование в рейд капитана\n"
                       "Например: ```css!!сбор Хомяк``` - отроет бронирование в рейд капитана Хомяк\n"
                       "```css!!сбор Хомяк 22:30``` - откроет бронирование в рейд капитана Хомяк,"
                       " который отплывает в 22:30\n"
                       "Нужно обладать ролью Капитан для использование этой команды")
help_msg_captain = ("Создаёт рейд\n"
                    "Например: ```css!!капитан Хомяк К-1 22:30 21:00 0```\n"
                    "Первый аргумент ```cssХомяк``` - имя капитана\n"
                    "Второй аргумент ```cssК-1``` - сервер, на котором будут отплывать на ежедневки\n"
                    "Третий аргумент ```css22:30``` - время отплытия на ежедневки\n"
                    "Четвёртый аргумент ```css21:00``` - открытие брони в рейд. Можно опустить\n"
                    "Пятый аргумент ```css0``` - количество зарезервированных дополнительных мест (всего будет 2 + *0*)"
                    ". Можно опустить\n"
                    "Нужно обладать ролью Капитан для использование этой команды")
msg_fail2 = (f"ТЫ НЕ ПОПАЛ В РЕЙД НА ЕЖИ\n"
             ...
             f"1) Написать мне сюда ```css!!рег [фамилия]``` (от слова регистрация), например *!!рег Хлебушек*."
