"""Data Driven Test to check user create by valid parameters with conflict."""
import pytest
...
async def test_ddt_request_with_conflict(test_data: dict, actual_create_user_response: SimpleResponse) -> None:
    """DDT to check user create by valid parameters with conflict.
    ...
    """
    soft_check_response_status_code(actual_create_user_response, codes.conflict)
    soft_check_response_json_attributes(
        actual_create_user_response,
        {"message": UsersAPIMessages.USER_CONFLICT},
